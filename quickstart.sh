...
prepare_data() {
  ...
  *)
  ...
  2)
    echo ""
    read -p "Sequential overlap을 입력하세요 (기본: 15): " sequential_overlap
    sequential_overlap=${sequential_overlap:-15}
    if ! [[ "$sequential_overlap" =~ ^[0-9]+$ ]] || [ "$sequential_overlap" -lt 1 ]; then
      echo "오류: sequential overlap은 1 이상의 정수여야 합니다."
      return 1
    fi

    colmap_args=(--colmap_matcher sequential --sequential_overlap "$sequential_overlap")

    echo ""
    echo "Sequential loop detection을 사용하시겠습니까?"
    echo "참고: 같은 공간으로 돌아오는 구간을 연결하는 데 도움이 될 수 있지만 COLMAP vocab tree 파일이 필요합니다."
    echo "vocab tree 파일이 없으면 N을 선택하세요."
    read -p "사용 여부 (y/N): " sequential_loop_choice
    case "$sequential_loop_choice" in
    y|Y|yes|YES)
      echo "vocab tree 파일의 상대 경로를 입력하세요 (예: data/vocab_tree_flickr100K_words256K.bin):"
      read -p "상대 경로: " vocab_tree_path

      if ! is_relative_path "$vocab_tree_path"; then
        echo "오류: 절대 경로는 사용할 수 없습니다. 상대 경로만 입력하세요."
        return 1
      fi

      if [ ! -f "$vocab_tree_path" ]; then
        echo "오류: vocab tree 파일을 찾을 수 없습니다: $vocab_tree_path"
        return 1
      fi

      colmap_args+=(--sequential_loop_detection 1 --vocab_tree_path "$vocab_tree_path")
      ;;
    *)
      ;;
    esac
  ...
  *)
  ...
  if [ -f "$scene_path/sparse/0/images.bin" ]; then
    cam_count=$(
      "$PYTHON_BIN" - "$scene_path/sparse/0/images.bin" <<'PY'
import sys
...
train_model() {
  ...
  camera_quality_ratio=$(
    "$PYTHON_BIN" - "$quality_ratio_percent" <<'PY'
import sys
